...
:: Author: Kia#0001
:: License: MIT
set WRAPPER_VER=0.1.0
title GoAnimaker v%WRAPPER_VER% [Initializing...]
...
title GoAnimaker v!WRAPPER_VER! [Checking dependencies...]
...
title GoAnimaker v!WRAPPER_VER! [Installing dependencies...]
		...
		if /i not !ERRORLEVEL!==0 (
			...
			if !FLASH_DETECTED!==n (
				if !NODEJS_DETECTED!==n (
					echo GoAnimaker needs to install Flash and Node.js.
				) else (
					echo GoAnimaker needs to install Flash.
				)
			) else (
				echo GoAnimaker needs to install Node.js.
			)
			echo To do this, it must be started with Admin rights.
			echo:
			echo Close this window and re-open GoAnimaker as an Admin.
...
if !FLASH_DETECTED!==n (
	...
	if !BROWSER_TYPE!==n (
		...
		echo GoAnimaker will not be able to install Flash.
	...
	:escape_browser_ask
	echo To install Flash Player, GoAnimaker must kill any currently running web browsers.
	echo Please make sure any work in your browser is saved before proceeding.
	echo GoAnimaker will not continue installation until you press a key.
	echo:
	pause
	echo:
	...
	if !BROWSER_TYPE!==chrome (
		...
		if not exist "utilities\installers\flash_windows_chromium.msi" (
			...
			echo A normal copy of GoAnimaker should come with one.
...
if !NODEJS_DETECTED!==n (
	...
	if !CPU_ARCHITECTURE!==64 (
		...
		echo You can just keep clicking next until it finishes, and GoAnimaker will continue once it closes.
	...
	if !CPU_ARCHITECTURE!==what (
		...
		echo GoAnimaker can't tell if you're on a 32-bit or 64-bit system.
	...
	if !NODEJS_DETECTED!==y (
		...
		if !errorlevel! == 0 (
			...
			if not exist "utilities\installers\http-server-master" (
				...
				echo A normal copy of GoAnimaker should come with them.
				echo You should be able to find a copy on this website:
		...
		echo Seems either the install failed, or GoAnimaker managed to skip it.
...
if !HTTPSCERT_DETECTED!==n (
	...
	if not exist "server\the.crt" (
		...
		echo GoAnimaker requires this to run.
		echo You should get a "the.crt" file from someone else, or redownload GoAnimaker.
	...
	if /i not "!SAFE_MODE!"=="y" (
		...
		if /i not !ERRORLEVEL!==0 (
			if !VERBOSEWRAPPER!==n ( cls )
			echo For GoAnimaker to work, it needs an HTTPS certificate to be installed.
...
if !ADMINREQUIRED!==y (
	...
	echo:
	echo GoAnimaker no longer needs Admin rights,
	echo please restart normally by double-clicking.
	echo:
	echo If you saw this from running normally,
	echo GoAnimaker should continue normally after a restart.
	echo:
	if !DRYRUN!==y (
...
color 0f
echo All dependencies now installed^^! Continuing with GoAnimaker boot.
echo:
...
title GoAnimaker v!WRAPPER_VER! [Loading...]
...
echo GoAnimaker has been started^^! The video list should now be open.

::::::::::::::::
:: Post-Start ::
::::::::::::::::

title GoAnimaker v!WRAPPER_VER!
...
echo:
echo GoAnimaker v!WRAPPER_VER! running
echo A project from the W:O team adapted by Kia
echo:
...
echo Enter 0 to close GoAnimaker
...
:exitwrapperconfirm
echo:
echo Are you sure you want to quit GoAnimaker?
...
title GoAnimaker v!WRAPPER_VER! [Shutting down...]
...
echo GoAnimaker has been shut down.
...
:exitwithstyle
title GoAnimaker v!WRAPPER_VER! [Shutting down... WITH STYLE]
...
echo :: GoAnimaker Config>> utilities\config.bat
